...
public class MinecraftRail implements ITrack, ITrackV2 {
	...
	public double[] getTrackGauges() {
		return new double[]{getTrackGauge()};
	}

	@Override
	public <D extends PathingData> void getNextPosition(D pos, Vec3 velocity, double gauge) {
		Vec3 next = getNextPosition(pos.getPos(), velocity);
		pos.setPos(next);
	}
