...
public class Util {
	...
	private static <T> T findInternal(final World world, Vec3 pos, boolean acceptMinecraftRails, Class<T> clazz) {
		int posX = (int) Math.floor(pos.xCoord);
		int posY = (int) Math.floor(pos.yCoord);
		int posZ = (int) Math.floor(pos.zCoord);
		...
		if (acceptMinecraftRails) {
			if (MinecraftRail.isRail(world, posX, posY, posZ) && clazz.isAssignableFrom(MinecraftRail.class)) {
				return clazz.cast(new MinecraftRail(world, posX, posY, posZ));
			}
		}
	...
	public static <T> T findTrackBlocks(World world, Vec3 pos, boolean acceptMinecraftRails, Class<T> clazz) {
		T track = findInternal(world, pos, acceptMinecraftRails, clazz);
		if (track != null) {
			return track;
		}
		// Allow a bit of vertical fuzziness
		track = findInternal(world, pos.addVector(0, 0.4, 0), acceptMinecraftRails, clazz);
		if (track != null) {
			return track;
		}
		track = findInternal(world, pos.addVector(0, -0.4, 0), acceptMinecraftRails, clazz);
